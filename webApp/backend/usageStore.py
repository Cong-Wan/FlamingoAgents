...
Version: 1.4
Date: 2026-08-19
Description: ~/.flamingo/logs/usage.db SQLite 用量统计库（契约 §3.10 / webAppPlan §11.4）：usageTurns 表 DDL 与增量写入（泵线程终态 delta，任一项 >0 才写）；
            ...
            v1.4 dbPath 迁至 ~/.flamingo/logs/usage.db；删除 backfillFromLogs（历史账由 logMigration 一次性搬库/merge）。
'''
...
from flamingoAgents.utils.logPaths import logsRoot, usageDbPath

from webApp.backend import modelConfigStore, sessionStore
...
def initUsageDb() -> None:
    # 服务启动完成前调用（__main__.py）：建表。历史账由 logMigration 搬到新库，此处不再从 jsonl 回填。
    getConnection()
